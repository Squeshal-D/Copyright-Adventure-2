...
public class Greed extends Character{

	ImageView portrait = new ImageView(new Image ("portraits/greedPortrait.jpg"));
	...
	String move2 = "";
	String move3 = "";
	String move1desc = "Does 20 damage";
	String move2desc = "Kills enemy if greed >= 1";
	String move3desc = "";
	...
	public void changeHP(int i)
	{
		...
		if (hp <= 0)
		{
			...
			for (int x = 0; x < Main.party.size(); x++) //removes greed statuses
			{
				for (int j = Main.party.get(x).getStatus().size() - 1; j >= 0; j--)
				{
					if (Main.party.get(x).getStatus().get(j).equals("greed"))
					{
						Main.party.get(x).getStatus().remove(j);
					}
				}
			}
		}
	...
	{
		String items[] = {"Lamp Oil", "Rope", "Bombs"};
		String item = items[(int) (Math.random()*3)];
		
		Timeline atk = new Timeline();
		int dmg = modifydmg(20);
		KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText(user.getName() + " threw " + item + " at "+ enemy.getName() + " for " + dmg + " dmg!"));
	...
	{
		Timeline atk = new Timeline();
		KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText("Death... You want it? It's yours, my friend."));
