...
public class Sloth extends Character{

	ImageView portrait = new ImageView(new Image ("portraits/slothPortrait.jpg"));
	...
	String move2 = "";
	String move3 = "";
	String move1desc = "Charges and does 60 damage";
	String move2desc = "";
	String move3desc = "";
	...
	public Timeline atk1(Character user, Character enemy, Label log, Stage ps)
	{
		...
		int dmg = modifydmg(60);
		if (!user.isCharging())
		{
			KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText(user.getName() + " is stockpiling energy for a deadly attack..."));
			charging = true;
			atk.getKeyFrames().add(first);
		}
		else
		{
			KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText(user.getName() + " poked " + enemy.getName() + " for " + dmg + " dmg!"));
			charging = false;
			atk.getKeyFrames().add(first);
			enemy.changeHP(-dmg);
		}
