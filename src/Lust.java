...
public class Lust extends Character{

	ImageView portrait = new ImageView(new Image ("portraits/lustPortrait.jpg"));
	...
	String move2 = "";
	String move3 = "";
	String move1desc = "Does 20 damage + weak status";
	String move2desc = "";
	String move3desc = "";
	...
	public Timeline atk1(Character user, Character enemy, Label log, Stage ps)
	{
		...
		if (r == 0)
		{
			KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText(user.getName() + " gives " + enemy.getName() + " the eyes."));
			atk.getKeyFrames().add(first);
		}
		else if (r == 1)
		{
			KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText(user.getName() + " flexed!"));
			atk.getKeyFrames().add(first);
		}
		else
		{
			KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText(user.getName() + " is being very handsome!"));
			atk.getKeyFrames().add(first);
		}
		enemy.changeHP(-dmg);
		enemy.addStat("weak");
