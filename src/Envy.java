...
public class Envy extends Character{

	ImageView portrait = new ImageView(new Image ("portraits/envyPortrait.jpg"));
	...
	String move2 = "";
	String move3 = "";
	String move1desc = "Steals the opponent's move and good statuses";
	String move2desc = "";
	String move3desc = "";
	...
	public Timeline atk1(Character user, Character enemy, Label log, Stage ps)
	{
		...
		if (enemy.getMoveName(1).equals("Punch")) //Classic Character
		{
			atk = enemy.atk1(user, enemy, log, ps);
		}
		else if (enemy.getName().equals("Paul Blart") || enemy.getName().equals("Master Chief") ||
				enemy.getName().equals("Derrek") || enemy.getName().equals("HULK") ||
				enemy.getName().equals("Harry Potter") || enemy.getName().equals("Scott") ||
				enemy.getName().equals("Spiderman") || enemy.getName().equals("John Wick"))
		{
			atk = enemy.atk1(user, enemy, log, ps);
		}
		else if (enemy.getName().equals("Drake") || enemy.getName().equals("Gandalf") ||
				enemy.getName().equals("Gordon Ramsay") || enemy.getName().equals("Shrek") ||
				enemy.getName().equals("Spongebob") || enemy.getName().equals("Tonio") ||
				enemy.getName().equals("George Washington"))
		{
			atk = enemy.atk2(user, enemy, log, ps);
		}
		else if (enemy.getName().equals("Caleb") || enemy.getName().equals("Josh") ||
				enemy.getName().equals("Gordon Freeman"))
		{
			atk = enemy.atk3(user, enemy, log, ps);
		}
		else
		{
			atk = null;
		...
		for (int i = enemy.getStatus().size() - 1; i >= 0; i--) //steals good statuses
		{
			if (enemy.getStatus().get(i).equals("heal") || enemy.getStatus().get(i).equals("rage"))
			{
				user.addStat(enemy.getStatus().get(i));
				enemy.getStatus().remove(i);
			}
		}
