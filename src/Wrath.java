...
public class Wrath extends Character{

	ImageView portrait = new ImageView(new Image ("portraits/wrathPortrait.jpg"));
	ImageView stage = new ImageView(new Image ("stages/scottStage.jpg"));
	String name = "League Player";
	...
	String move2 = "";
	String move3 = "";
	String move1desc = "Does 20 damage and adds permanent rage";
	String move2desc = "";
	String move3desc = "";
	...
	public Timeline atk1(Character user, Character enemy, Label log, Stage ps)
	{
		...
		if (enemy.getMoveDesc(2).equals("No, it's not hitting birds.\nDoes 50 damage, but takes a turn to charge."
				+ "\nThe charge is interrupted if you take 20+ damage in a single attack."))
		{
			move2desc = "done";
			KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText(user.getName() + " showed " + enemy.getName() + " the true power of league!"));
			KeyFrame second = new KeyFrame(Duration.seconds(2), ae -> {
				
				log.setText(enemy.getName() + "'s Falcon Punch was upgraded!");
			...
			KeyFrame third = new KeyFrame(Duration.seconds(5));
			atk.getKeyFrames().addAll(first, second, third);
			enemy.changeHP(enemy.getMaxHP());
		}
		else
		{
			KeyFrame first = new KeyFrame(Duration.ZERO, ae -> log.setText(user.getName() + " got mad for no reason! " + enemy.getName() + " took " + dmg + " dmg!"));
